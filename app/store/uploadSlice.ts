...
const uploadSlice = createSlice({
  ...
  reducers: {
    ...
    clearPendingFile: (
      ...
      const { [uploadId]: targetUpload, ...remainingUploads } =
        state.pendingFiles;
  ...
  extraReducers: (builder) => {
    ...
    builder
      .addCase(uploadImage.pending, (state, action) => {
        const { uploadId, file, width, height }: FileUploadRequest =
          action.meta.arg;
...
export const uploadImage = createAsyncThunk<
  FileUploadSuccess,
  FileUploadRequest,
  { rejectValue: FileUploadFailure }
>("upload/uploadImage", async (imageData, { rejectWithValue }) => {
  ...
  try {
    ...
    const errorMessage =
      e instanceof ApiResponseError
        ? e.data
        : "Oops! Image upload failed. Please try again";
