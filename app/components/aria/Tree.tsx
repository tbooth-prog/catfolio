...
const itemStyles = tv({
  extend: focusRing,
  base: "relative font-sans flex group gap-3 cursor-default select-none py-1 px-3 text-sm text-neutral-900 dog:text-neutral-200 bg-white dog:bg-neutral-900 border-t dog:border-t-neutral-700 border-transparent first:border-t-0 -outline-offset-2 first:rounded-t-lg last:rounded-b-lg",
  variants: {
    isSelected: {
      false:
        "hover:bg-neutral-100 pressed:bg-neutral-100 dog:hover:bg-neutral-800 dog:pressed:bg-neutral-800",
  ...
  return (
    <AriaTree
      ...
      className={composeTailwindRenderProps(
        props.className,
        "w-48 max-w-full overflow-auto relative border border-neutral-200 dog:border-neutral-700 rounded-lg",
      )}
    >
      {children}
    </AriaTree>
...
const chevron = tv({
  base: "w-4.5 h-4.5 text-neutral-500 dog:text-neutral-400 transition-transform duration-200 ease-in-out",
  variants: {
    isExpanded: {
